# Reproducing the Seed99 Bootstrap Confidence Intervals Run

## Important Note

The repository does not expose the full three-method Seed99 pipeline as a single packaged CLI command. The exact reproducible run is a sequence of service calls. The command block below reproduces the saved `seed99_robustness_*` formal experiment artefacts from the current codebase, including the bootstrap confidence interval report.

## Exact Seed99 Bootstrap CI Run Command
